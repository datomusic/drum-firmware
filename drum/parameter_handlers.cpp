...
TempoParameterHandler::TempoParameterHandler(
    musin::timing::TempoHandler &tempo_handler, MessageRouter &message_router)
...
PitchParameterHandler::PitchParameterHandler(MessageRouter &message_router,
                                             uint8_t track_index)
    : message_router_(message_router), track_index_(track_index) {
}

void PitchParameterHandler::handle_update(float normalized_value) {
  message_router_.set_parameter(drum::Parameter::PITCH, normalized_value,
                                track_index_);
}
...
SwingParameterHandler::SwingParameterHandler(
    DefaultSequencerController &sequencer_controller,
...
void SwingParameterHandler::handle_update(float normalized_value) {
  float distance_from_center = fabsf(
      normalized_value - config::analog_controls::SWING_KNOB_CENTER_VALUE);
...
void CrushParameterHandler::handle_update(float normalized_value) {
  message_router_.set_parameter(drum::Parameter::CRUSH_EFFECT,
                                normalized_value);
}

// --- RandomParameterHandler ---
RandomParameterHandler::RandomParameterHandler(
    DefaultSequencerController &sequencer_controller,
    MessageRouter &message_router)
    : sequencer_controller_(sequencer_controller),
      message_router_(message_router) {
...
void RandomParameterHandler::handle_update(float normalized_value) {
  ...
  bool should_be_active =
      (normalized_value >=
       config::analog_controls::RANDOM_ACTIVATION_THRESHOLD);
  ...
  message_router_.set_parameter(drum::Parameter::RANDOM_EFFECT,
                                normalized_value, 0);
}

// --- RepeatParameterHandler ---
RepeatParameterHandler::RepeatParameterHandler(
    DefaultSequencerController &sequencer_controller,
    MessageRouter &message_router)
    : sequencer_controller_(sequencer_controller),
      message_router_(message_router) {
...
void RepeatParameterHandler::handle_update(float normalized_value) {
  ...
  } else if (normalized_value >=
             config::analog_controls::REPEAT_MODE_1_THRESHOLD) {
  ...
  message_router_.set_parameter(drum::Parameter::REPEAT_EFFECT,
                                normalized_value);
}

// --- FilterParameterHandler ---
FilterParameterHandler::FilterParameterHandler(float &target_value)
