...
enum {
  USBD_ITF_CDC = 0, // CDC needs 2 interfaces
  USBD_ITF_CDC_DATA,
  ITF_NUM_MIDI,
...
// On Windows, if bcdUSB = 0x210 then a Microsoft OS 2.0 descriptor is required,
// else the device won't be detected This is only needed for driverless access
// to the reset interface - the CDC interface doesn't require these descriptors
...
#define USBD_DEVICE_BCD                                                        \
  0x0100 // Default bcdDevice 1.00 if not set by build system
#endif
