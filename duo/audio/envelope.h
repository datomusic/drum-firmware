...
Example:
  ...
  Each time we update our samples we do something like this:
    ...
    int val = envelope.curVal; // Kepp local value to piecewise increment for
each sample for (int i = 0; i < 8; ++i) { setSampleAmplitude(samples[i], val);
      val += inc;
    }
*/

struct LinearEnvelope {
  LinearEnvelope(int maxValue, int subdivisionCount)
      : maxValue(maxValue), subdivisionCount(subdivisionCount),
        conf(maxValue, 0, 0, 0, 0) {
  }
  ...
  int curVal = 0; // Should NOT be externally modified, but depending on
                  // compiler optimizations it might be faster to keep it as
                  // public member.
};
