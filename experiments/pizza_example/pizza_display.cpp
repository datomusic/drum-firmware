...
PizzaDisplay::PizzaDisplay() :
    leds(PIN_LED_DATA, Musin::Drivers::RGBOrder::GRB, 255, 0xffe080), // Initial brightness 255, will be adjusted in init
    note_colors({
        0xFF0000, 0xFF0020, 0xFF0040, 0xFF0060,
        0xFF1010, 0xFF1020, 0xFF2040, 0xFF2060,
        0x0000FF, 0x0028FF, 0x0050FF, 0x0078FF,
        0x1010FF, 0x1028FF, 0x2050FF, 0x3078FF,
        0x00FF00, 0x00FF1E, 0x00FF3C, 0x00FF5A,
        0x10FF10, 0x10FF1E, 0x10FF3C, 0x20FF5A,
        0xFFFF00, 0xFFE100, 0xFFC300, 0xFFA500,
...
uint32_t PizzaDisplay::get_drumpad_led_index(uint8_t pad_index) const {
    switch (pad_index) {
        ...
        case 1: return LED_DRUMPAD_2;
        case 2: return LED_DRUMPAD_3;
        case 3: return LED_DRUMPAD_4;
        default: return NUM_LEDS; // Invalid index
    }
}
...
void PizzaDisplay::set_keypad_led(uint8_t row, uint8_t col, uint8_t intensity) {
    ...
    if (array_index < std::size(LED_ARRAY)) {
        ...
        uint32_t color = (static_cast<uint32_t>(brightness_val) << 16) |
                         (static_cast<uint32_t>(brightness_val) << 8)  |
                         brightness_val;
