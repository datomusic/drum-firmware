...
DrumPizza::DrumPizza(const std::array<std::uint32_t, 4> &address_pins_gpio,    // Use std::uint32_t
    ...
    : // Initialize _key_data_buffer implicitly (default constructor for array elements)
      _keypad(
          KEYPAD_ROWS, // Member initialization order matters
          KEYPAD_COLS,
          ...
          // This is slightly less efficient but ensures type safety without changing Keypad_HC138
          // yet.
          ...
          [keypad_col_pins_gpio]() -> std::array<unsigned int, 5> {
            ...
            for (size_t i = 0; i < 5; ++i)
              temp_cols[i] = static_cast<unsigned int>(keypad_col_pins_gpio[i]);
            return temp_cols;
          }()
                                          .data(), // Pass pointer to the temporary array's data
          _key_data_buffer.data(),                 // Pass pointer to the internal buffer
          scan_interval_us, debounce_time_us, hold_time_us),
...
static ExternalPinState check_external_pin_state(std::uint32_t gpio,
                                                 const char *name) { // Use std::uint32_t
  gpio_init(gpio);
