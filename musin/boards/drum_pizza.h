...
public:
   ...
   *
   * @param address_pins_gpio Array of 4 GPIO pin numbers corresponding to NamedPin::ADDR_0, ADDR_1,
   * ADDR_2, ADDR_3. The first 3 are used by the keypad decoder, all 4 might be used by other
   * components (e.g., analog mux).
   * @param keypad_col_pins_gpio Array of 5 GPIO pin numbers corresponding to NamedPin::RING1 to
   * RING5.
   * @param led_data_pin_gpio GPIO pin number corresponding to NamedPin::LED_DATA.
   * @param led_data_return_pin_gpio Optional GPIO pin number corresponding to
   * NamedPin::LED_DATA_RETURN.
   ...
   */
  DrumPizza(
      const std::array<std::uint32_t, 4> &address_pins_gpio,                // Use std::uint32_t
   ...
   */
  const musin::ui::Keypad_HC138 &keypad() const {
    return _keypad;
  }
   ...
   */
  musin::drivers::WS2812<musin::Boards::NUM_LEDS> &leds() {
    return _leds;
  }
   ...
   */
  const musin::drivers::WS2812<musin::Boards::NUM_LEDS> &leds() const {
    return _leds;
  }
  ...
  // static constexpr NamedPin PIN_LED_DATA_RETURN = NamedPin::LED_DATA_RETURN; // Optional (J1 Pin
  // 18)
