 ...
 */
template <size_t NUM_LEDS> class WS2812 {
...
public:
   ...
   */
  WS2812(unsigned int data_pin, RGBOrder order = RGBOrder::GRB, uint8_t initial_brightness = 255,
...
WS2812<NUM_LEDS>::WS2812(unsigned int data_pin, RGBOrder order, uint8_t initial_brightness,
    ...
    : _pio(nullptr),       // Initialized in init()
      ...
      _order(order), _brightness(initial_brightness), _color_correction(color_correction),
      _pixel_buffer{} // Default-initialize array elements to 0 (black)
{
  // No assertion needed for NUM_LEDS > 0 due to static_assert
}

template <size_t NUM_LEDS> bool WS2812<NUM_LEDS>::init() {
  if (_initialized) {
    return true;
  }
...
template <size_t NUM_LEDS> void WS2812<NUM_LEDS>::set_pixel(unsigned int index, uint32_t color) {
  if (!_initialized || index >= NUM_LEDS) {   // Use NUM_LEDS for check
    assert(index < NUM_LEDS && _initialized); // Optional: Assert in debug builds
    return;
  }
...
template <size_t NUM_LEDS> void WS2812<NUM_LEDS>::show() {
  if (!_initialized) {
    assert(_initialized);
    return;
  }
...
template <size_t NUM_LEDS> void WS2812<NUM_LEDS>::clear() {
  if (!_initialized) {
...
template <size_t NUM_LEDS> void WS2812<NUM_LEDS>::fade_by(uint8_t fade_amount) {
  if (!_initialized) {
    assert(_initialized); // Optional: Assert in debug builds
    return;
  }
...
template <size_t NUM_LEDS> void WS2812<NUM_LEDS>::set_brightness(uint8_t brightness) {
  // Store brightness; applied during the next set_pixel call.
  _brightness = brightness;
}

template <size_t NUM_LEDS> uint8_t WS2812<NUM_LEDS>::get_brightness() const {
  return _brightness;
}

template <size_t NUM_LEDS> constexpr size_t WS2812<NUM_LEDS>::get_num_leds() const {
  return NUM_LEDS;
}
...
template <size_t NUM_LEDS>
void WS2812<NUM_LEDS>::unpack_color(uint32_t packed_color, uint8_t &r, uint8_t &g,
                                    uint8_t &b) const {
  switch (_order) {
  case RGBOrder::RGB:
