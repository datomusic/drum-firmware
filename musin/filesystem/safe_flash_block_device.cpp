...
static int safe_read(blockdevice_t *device, const void *buffer, bd_size_t addr, bd_size_t size) {
    safe_flash_config_t *config = (safe_flash_config_t *)device->config;
    
    uint32_t xip_addr = XIP_NOCACHE_NOALLOC_NOTRANSLATE_BASE + safe_flash_target_offset(device) + (size_t)addr;
    
    // Only log reads >= 256 bytes or sector operations
    if (size >= 256 || (addr % FLASH_SECTOR_SIZE) == 0) {
        printf("PRINTF: safe_read: addr=0x%08lx, size=0x%08lx, xip_addr=0x%08lx\n",
               (unsigned long)addr, (unsigned long)size, (unsigned long)xip_addr);
    }
...
static int safe_erase(blockdevice_t *device, bd_size_t addr, bd_size_t size) {
    size_t flash_addr = safe_flash_target_offset(device) + addr;
    printf("PRINTF: safe_erase: addr=0x%08lx, size=0x%08lx, flash_addr=0x%08lx\n",
           (unsigned long)addr, (unsigned long)size, (unsigned long)flash_addr);
    ...
    int err = flash_safe_execute(_safe_flash_update, &param, SAFE_FLASH_EXECUTE_TIMEOUT);
    printf("PRINTF: safe_erase result: %d\n", err);
    return _error_remap(err);
}

static int safe_program(blockdevice_t *device, const void *buffer, bd_size_t addr, bd_size_t size) {
    size_t flash_addr = safe_flash_target_offset(device) + addr;
    
    // Always log program operations as they're less frequent and more critical
    printf("PRINTF: safe_program: addr=0x%08lx, size=0x%08lx, flash_addr=0x%08lx\n",
           (unsigned long)addr, (unsigned long)size, (unsigned long)flash_addr);
    ...
    int err = flash_safe_execute(_safe_flash_update, &param, SAFE_FLASH_EXECUTE_TIMEOUT);
    printf("PRINTF: safe_program result: %d\n", err);
    return _error_remap(err);
}
...
static bd_size_t safe_size(blockdevice_t *device) {
    safe_flash_config_t *config = (safe_flash_config_t *)device->config;
    bd_size_t size = (bd_size_t)config->length;
    printf("PRINTF: safe_size returning: 0x%08lx bytes (%ld MB)\n", 
           (unsigned long)size, (unsigned long)size / (1024*1024));
    return size;
}

blockdevice_t *safe_flash_block_device_create(uint32_t flash_offset, size_t size) {
    printf("PRINTF: safe_flash_block_device_create called with offset=0x%08lx, size=0x%08lx\n",
           (unsigned long)flash_offset, (unsigned long)size);
    ...
    safe_flash_config_t *config = (safe_flash_config_t *)calloc(1, sizeof(safe_flash_config_t));
    if (config == NULL) {
        fprintf(stderr, "safe_flash_block_device_create: Out of memory\n");
        free(device);
        return NULL;
