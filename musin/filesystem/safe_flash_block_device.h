 ...
 * 
 * @param flash_offset The flash offset (not XIP address) where the partition starts
 * @param size Size of the partition in bytes
 * @return blockdevice_t* Pointer to the created block device, or nullptr on failure
 */
blockdevice_t *safe_flash_block_device_create(uint32_t flash_offset, size_t size);

/**
 * @brief Frees a safe flash block device created with safe_flash_block_device_create
 * 
