...
void MidiSender::sendNoteOn(uint8_t channel, uint8_t note_number,
                            uint8_t velocity) {
  if (_strategy == MidiSendStrategy::DIRECT_BYPASS_QUEUE) {
    ...
    enqueue_midi_message(
        OutgoingMidiMessage(channel, note_number, velocity, true), _logger);
  }
}

void MidiSender::sendNoteOff(uint8_t channel, uint8_t note_number,
                             uint8_t velocity) {
  if (_strategy == MidiSendStrategy::DIRECT_BYPASS_QUEUE) {
    ...
    enqueue_midi_message(
        OutgoingMidiMessage(channel, note_number, velocity, false), _logger);
  }
}
