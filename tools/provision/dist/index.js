...
async function runCommand(command, args, logger, options = {}) {
...
async function waitForBootsel(timeoutSeconds, logger) {
...
async function waitForVolume(volumeName, timeoutSeconds, logger) {
...
async function waitForUsbDevice(deviceName, timeoutSeconds, logger, cwd) {
    logger.info(`Waiting for USB device '${deviceName}' to appear...`);
    const start = Date.now();
    while (true) {
        const result = await runCommand('system_profiler', ['SPUSBDataType'], logger, { silent: true, ignoreFailure: true, cwd });
...
async function runProvision({ useJson }) {
    ...
    try {
        ...
        for (const tool of requiredTools) {
            const toolResult = (0, child_process_1.spawn)('which', [tool], { stdio: 'pipe' });
        ...
        try {
            await runCommand('picotool', ['otp', 'white-label', '-s', '0x400', 'drum/white-label.json', '-f'], logger, { cwd: projectRoot });
        ...
        if (!partitionUf2Path) {
            ...
            if (!partitionUf2Path && fs.existsSync(partitionJsonPath)) {
                ...
                await runCommand('picotool', ['partition', 'create', partitionJsonPath, partitionUf2Path], logger, { cwd: projectRoot });
            }
            if (!partitionUf2Path) {
        ...
        if (!fs.existsSync(samplesPath)) {
            ...
            if (files.length === 0) {
                ...
                const result = await runCommand('node', ['tools/drumtool/drumtool.js', 'send', ...files], logger, { cwd: projectRoot, ignoreFailure: true });
                if (result.exitCode === 0) {
